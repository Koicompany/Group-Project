...
public class WASDPlayerAttack : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Melee attack - key T
        if (Input.GetKeyDown(KeyCode.T) && cooldownTimer > attackCooldown && !isAttacking)
        {
            StartCoroutine(MeleeAttack());
        }

        // Ranged attack - key Y
        if (Input.GetKeyDown(KeyCode.Y) && cooldownTimer > projectileCooldown && !isAttacking)
        {
    ...
    private void DamageEnemy()
    {
        ...
        if (hit.collider != null)
        {
            ...
            if (enemyHealth != null && enemyID != null)
            {
                ...
                // Apply weaknesses/advantages if attacker has the manager
                ...
                Debug.Log($"{GetComponent<CharacterID>().characterName} damaged {enemyID.characterName} for {finalDamage}");
            }
        }
    }
